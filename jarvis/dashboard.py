...
def render_dashboard(state: State, monitor: Monitor = None, d: Defaults = None,
                     hermes_status: dict = None) -> str:
    d = d or Defaults()
    ...
    hm = "🟢 UP" if (hermes_status or {}).get("running") else "🔴 DOWN"
    if hermes_status and hermes_status.get("launched"):
        hm = "🟡 LAUNCHED"

    lines = []
    lines.append("=" * 40)
    lines.append("        J A R V I S  (supervisor)")
    lines.append("=" * 40)
    lines.append(f"Status : 🟢 ONLINE  (cycle #{state.get_cycle()})")
    lines.append(f"Hermes : {hm}   <- Jarvis keeps it alive")
    ...
    lines.append(f"Disk   : {health.get('disk_free_mb', 0)} MB free")
    lines.append(f"Net    : {'🟢' if health.get('online') else '🔴 OFFLINE'}")
    lines.append(f"Spawn? : {'yes' if health['can_spawn'] else 'NO (guard)'}")
    lines.append("-" * 40)
    for t in running[:5]:
        lines.append(f"[{t.status.value:>6}] {t.sub_goal[:44]}")
    if len(running) > 5:
